...
- `deno.lock`: **always revert it** (`git checkout deno.lock`), never commit a diff, even for PRs that intentionally add Deno deps (zod mapping, satori/resvg/jpeg). Corrected from rev-2: any `deno test --node-modules-dir=auto` run rewrites deno.lock as a full snapshot of the *entire* root `package.json` dependency graph (thousands of lines), not a surgical diff for the new import — there is no clean "intentional" version to keep. This is safe to discard because `.github/workflows/ci.yml` runs plain `npm run test:functions` with no lockfile-freeze flag on a fresh checkout, so CI resolves dependencies fresh every run regardless of what's committed. After any local `deno test`/`deno fmt`/`deno check` invocation: `git checkout deno.lock && npm ci` — and if a build afterward shows cross-version type errors (e.g. two versions of the same package clashing), that means a stray `node_modules/.deno/<pkg>@<version>` directory survived; `npm ci` alone doesn't always clear those, so re-run it (npm ci does a from-scratch `node_modules` wipe+reinstall) rather than trying to hand-delete the specific stray directory.
...
Chunked calls re-read row (abort 204 on rev/status change); self-invocation via **`EdgeRuntime.waitUntil`** (injectable); finalize only on last page; sweep cron re-fires stuck rows >2 min, queues `file_deletions` for superseded-rev manifests it reaps, cron-failure email; config.toml + config-audit entry; migration `20260702000004` (vault-view schedule, every 2 min) applied **after** deploys.
