...
- `deno.lock`: PRs that **intentionally** add Deno deps (zod mapping, satori/resvg/jpeg) commit the updated lockfile. `git checkout deno.lock && npm ci` is only the recovery for incidental pollution from local runs.
