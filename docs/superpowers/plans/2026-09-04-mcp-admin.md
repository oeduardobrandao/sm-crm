...
export async function updateBanner(d: Deps, args: Record<string, unknown>) {
  ...
  const err = validateBanner({ ...(current as Record<string, unknown>), ...update });
  if (err) throw new McpInputError(err);
...
export async function updateKbArticle(d: Deps, args: Record<string, unknown>) {
  ...
  const err = validateKbArticle({ ...(current as Record<string, unknown>), ...update });
  if (err) throw new McpInputError(err);
...
Antes do `db push` em prod, rodar a query de pré-checagem do CHECK dos banners (Tarefa 1, Step 1b). Repetir para staging (ref `wlyzhyfondykzpsiqsce`), aplicando primeiro a migration de popups que falta lá.
