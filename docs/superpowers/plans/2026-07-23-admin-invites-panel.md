...
- **Gates before pushing:** `npm run test`, `npm run build:admin`, `deno test supabase/functions/`, `npm run lint`, `npm run format:check`. After the deno run, restore `deno.lock` only if the run dirtied *that file alone* (`git checkout -- deno.lock`).
...
- `supabase/functions/invite-user/onboarding.ts`, `invite-user/pending-invite.ts` — DELETE (moved)
- `supabase/functions/platform-admin/index.ts` — MODIFY (3 actions + handlers)
- Tests: `supabase/functions/__tests__/invite-user-onboarding_test.ts` (retarget import), `invite-user-pending_test.ts` (retarget import), `invite-actions_test.ts` (CREATE), `platform-admin-invites_test.ts` (CREATE)
...
Run: `deno test supabase/functions/__tests__/invite-user-onboarding_test.ts`
Expected: all 13 tests PASS.

- [ ] **Step 6: Commit**

```bash
git checkout -- supabase/functions/deno.lock 2>/dev/null || true
...
Run: `deno test supabase/functions/__tests__/invite-user-pending_test.ts`
...
The single invite-or-resend primitive. Order: (1) seat pre-check (same counts/rule as `invite-user`); (2) delete prior pending/expired rows for this email+conta; (3) classify the existing auth user; (4) route. `add-direct` **adds the member** (finding 2), never a delete-and-no-grant.
...
- Produces: `inviteOrResend(adminClient, input): Promise<InviteOutcome>` where
  `input = { contaId: string; email: string; role: "owner"|"admin"|"agent"; invitedBy: string; redirectBase: string }`,
  `InviteOutcome = { route: "added" | "already-member" | "resent-link" | "reinvited" | "invited" | "plan-limit-exceeded" | "blocked-anomalous" }`.
...
Run: `grep -rn "invite-user/seats" supabase/functions apps` → expect no output. Then `deno test supabase/functions/__tests__/invite-user-seats_test.ts` → PASS.
  ...
  return {
    ...
    auth: {
      admin: {
        ...
        deleteUser: (id: string) => { events.push("delUser:" + id); return Promise.resolve({ error: null }); },
    ...
    from: (table: string) => {
      const api: any = {
        select: (_c?: string, o?: any) => { if (o?.head) { api._count = table === "workspace_members" ? opts.members : opts.pending; } return api; },
        eq: () => api,
        in: () => api,
        delete: () => { events.push("del:" + table); return api; },
        insert: (row: any) => { events.push("ins:" + table + ":" + (row.status ?? "")); return { select: () => ({ single: () => Promise.resolve({ data: { id: "new-invite" }, error: null }) }) }; },
        maybeSingle: () => {
        ...
        then: (r: (x: any) => unknown) => Promise.resolve(r({ count: api._count ?? 0, data: null, error: null })),
      };
      return api;
    },
  };
...
Deno.test("inviteOrResend: over-limit resend is rejected before any send", async () => {
  const admin = makeInviteAdmin({ limit: 3, members: 3, pending: 0 });
  // deno-lint-ignore no-explicit-any
  const out = await inviteOrResend(admin as any, baseInput);
...
Deno.test("inviteOrResend: brand-new email sends an auth invite", async () => {
  const admin = makeInviteAdmin({ limit: null, members: 1, pending: 0, authUser: null });
  // deno-lint-ignore no-explicit-any
  const out = await inviteOrResend(admin as any, baseInput);
...
Deno.test("inviteOrResend: confirmed-but-passwordless gets a fresh recovery link", async () => {
  // The resend-link route emails via Resend; stub the env + fetch so the HTTP
  // call succeeds without a real key. Restore both afterwards.
  ...
  try {
    const admin = makeInviteAdmin({ limit: null, members: 1, pending: 1, authUser: { id: "u1", email_confirmed_at: "2026-01-01T00:00:00Z" }, hasProfile: true, onboarding: false, hasPassword: false });
    // deno-lint-ignore no-explicit-any
    const out = await inviteOrResend(admin as any, baseInput);
...
Deno.test("inviteOrResend: onboarded non-member is ADDED, not left without access", async () => {
  const admin = makeInviteAdmin({ limit: null, members: 1, pending: 0, authUser: { id: "u1", email_confirmed_at: "2026-01-01T00:00:00Z" }, hasProfile: true, onboarding: true, hasPassword: true, isMember: false });
  // deno-lint-ignore no-explicit-any
  const out = await inviteOrResend(admin as any, baseInput);
...
Deno.test("inviteOrResend: onboarded existing member is a no-op already-member", async () => {
  const admin = makeInviteAdmin({ limit: null, members: 1, pending: 0, authUser: { id: "u1", email_confirmed_at: "2026-01-01T00:00:00Z" }, hasProfile: true, onboarding: true, hasPassword: true, isMember: true });
  // deno-lint-ignore no-explicit-any
  const out = await inviteOrResend(admin as any, baseInput);
  assertEquals(out.route, "already-member");
});

Deno.test("inviteOrResend: never-confirmed stale user is reinvited fresh", async () => {
  const admin = makeInviteAdmin({ limit: null, members: 1, pending: 1, authUser: { id: "u1", email_confirmed_at: null }, hasProfile: true, onboarding: false, hasPassword: false });
  // deno-lint-ignore no-explicit-any
  const out = await inviteOrResend(admin as any, baseInput);
...
export type InviteRoute =
  | "added" | "already-member" | "resent-link" | "reinvited" | "invited"
  | "plan-limit-exceeded" | "blocked-anomalous";
export interface InviteOutcome { route: InviteRoute; }

/**
 * THE invite-or-resend primitive shared by invite-user (CRM) and
 * admin-resend-invite (portal). Faithful port of invite-user's inline POST
 * logic; the only added guarantee is that add-direct ADDS the member (never a
 * delete-and-no-grant) and that the seat check runs on every path.
 */
export async function inviteOrResend(
  ...
  // (1) Seat pre-check — identical counts/rule to invite-user today.
  ...
  const [{ count: members }, { count: pending }] = await Promise.all([
    ...
    adminClient.from("invites").select("*", { count: "exact", head: true })
      .eq("conta_id", input.contaId).eq("status", "pending"),
  ]);
  if (!seatsAvailable({ limit, members: members ?? 0, pendingInvites: pending ?? 0 })) {
    return { route: "plan-limit-exceeded" };
  }

  // (2) Clean up prior pending/expired rows for this email+conta.
  await adminClient.from("invites").delete()
    .eq("email", email).eq("conta_id", input.contaId).in("status", ["pending", "expired"]);

  // (3) Classify the existing auth user (if any).
  ...
  if (existingUser) {
    ...
    if (action === "reinvite") {
      ...
      return { route: "reinvited" };
    }

    // add-direct: fully onboarded. Add to the workspace (never no-op-destroy).
    const { data: membership } = await adminClient
      .from("workspace_members").select("id").eq("user_id", existingUser.id).eq("workspace_id", input.contaId).maybeSingle();
    if (membership) return { route: "already-member" };

    await adminClient.from("workspace_members").insert({
      user_id: existingUser.id, workspace_id: input.contaId, role: input.role,
    });
...
**Files:**
- Modify: `supabase/functions/platform-admin/index.ts` (dispatch + new handler)
...
- Produces: handler `handleGetWorkspaceInvites(svc, body, headers)`; response `{ invites: EnrichedInvite[]; total: number }` where
...
Deno.test("computeInviteFlags marks a pending invite link expired after 24h", () => {
  const now = new Date("2026-07-23T13:00:00.000Z").getTime();
  const f = computeInviteFlags(
    { status: "pending", created_at: "2026-07-21T12:00:00.000Z", accepted_at: null },
    "2026-07-21T12:00:00.000Z", // confirmation_sent_at > 24h ago
    now,
  );
  assertEquals(f.link_expired, true);
});

Deno.test("computeInviteFlags: pending with a recent send is not expired", () => {
  const now = new Date("2026-07-23T13:00:00.000Z").getTime();
  const f = computeInviteFlags(
    { status: "pending", created_at: "2026-07-23T12:00:00.000Z", accepted_at: null },
...
Run: `deno test supabase/functions/__tests__/platform-admin-invites_test.ts`
 ...
 * link_expired: a pending invite whose email was dispatched (confirmation_sent_at)
 *   more than 24h ago — the GoTrue link lifetime, not the cosmetic expires_at.
 */
export function computeInviteFlags(
  ...
  const link_expired = invite.status === "pending" && confirmationSentAt != null &&
    (now - new Date(confirmationSentAt).getTime()) > LINK_TTL_MS;
...
- [ ] **Step 5: Add the handler + dispatch case**

In `supabase/functions/platform-admin/index.ts`, add the import near the top:
...
Add the dispatch case in the `switch (action)` block (next to the other workspace reads):
...
Add the handler (place it near `handleListWorkspaceMcpKeys`):

```ts
async function handleGetWorkspaceInvites(
  ...
  const enriched = invites.map((r) => {
    ...
    const flags = computeInviteFlags(r, auth?.confirmation_sent_at ?? null);
    return {
      ...r,
...
```bash
git checkout -- supabase/functions/deno.lock 2>/dev/null || true
git add supabase/functions/platform-admin/index.ts supabase/functions/platform-admin/invites-enrich.ts supabase/functions/__tests__/platform-admin-invites_test.ts
...
The two mutating actions, audit-logged. Cancel writes one audit row per affected workspace sharing an `operation_id` (finding 5); resend records the route.

**Files:**
- Modify: `supabase/functions/platform-admin/index.ts` (dispatch + two handlers)
- Modify: `supabase/functions/__tests__/platform-admin-invites_test.ts` (route→message + audit mapping tests)
...
Deno.test("resendMessage maps added/already-member to 200 info", () => {
  assertEquals(resendMessage("added").status, 200);
  assert(String(resendMessage("added").body.message).length > 0);
...
export function resendMessage(route: InviteRoute): { status: number; body: Record<string, unknown> } {
  switch (route) {
    ...
    case "added":
      return { status: 200, body: { success: true, route, message: "User already had an account and was added to the workspace." } };
    case "already-member":
...
- [ ] **Step 5: Add the two handlers + dispatch cases**

In `supabase/functions/platform-admin/index.ts` add imports:
...
(Combine with the Task 7 `getAuthStatesByEmails` import from the same module into one import line.)
...
Add the handlers. `operation_id` is derived with `crypto.randomUUID()` (available in the Deno edge runtime):

```ts
async function handleAdminCancelInvite(
  svc: ReturnType<typeof createClient>,
...
async function handleAdminResendInvite(
  svc: ReturnType<typeof createClient>,
  body: { workspace_id?: string; invite_id?: string },
  adminUserId: string,
  headers: Record<string, string>,
  ...
  if (mapped.status < 300) {
    await insertAuditLog(svc, {
      ...
      conta_id: body.workspace_id,
      actor_user_id: adminUserId,
      resource_type: "invite",
      resource_id: body.invite_id,
      metadata: { email: invite.email, route: outcome.route },
    });
  }
...
- [ ] **Step 6: Typecheck + full edge suite**

Run: `deno check supabase/functions/platform-admin/index.ts && deno test supabase/functions/`
...
import { getWorkspaceInvites } from '../../lib/api';
...
describe('WorkspaceInvitesCard', () => {
  ...
  it('shows Resend + Cancel for a pending invite', async () => {
    (getWorkspaceInvites as any).mockResolvedValue({ invites: [inv({ status: 'pending' })], total: 1 });
    renderCard();
    expect(await screen.findByRole('button', { name: /resend/i })).toBeTruthy();
...
function InviteRow({ invite, busy, onResend, onCancel }: {
  ...
  return (
    <div className="min-w-0 border-b border-border/50 py-2.5 md:grid md:grid-cols-[2fr_0.8fr_1.4fr_1.6fr_1fr] md:gap-2 md:items-center">
      <div className="min-w-0">
        <span className="block truncate text-sm">{invite.email}</span>
        {tags.map((t) => (
          <span key={t} className="mt-0.5 inline-block text-[0.6rem] font-semibold uppercase px-1.5 py-0.5 rounded-sm bg-warning/10 text-warning">
            {t}
          </span>
        ))}
      </div>
...
Expected: all green. If `deno test` dirtied only `deno.lock`, run `git checkout -- deno.lock` (never blanket-discard other changes).
...
- [ ] **Step 3: Deploy both edge functions together (--use-api)**

Confirm the linked project ref first (`cat supabase/.temp/project-ref`, translate against prod `skjzpekeqefvlojenfsw` / staging `wlyzhyfondykzpsiqsce`), then:

```bash
npx supabase functions deploy invite-user --use-api --project-ref <ref>
