...
- **Seat rule:** `inviteOrResend` enforces `max_team_members` for every route. Order: (1) seat pre-check — counts members + pending invites **excluding any pending row for this same email** (a resend replaces that row, consuming no new seat); (2) classify the existing auth user; (3) route, deleting prior pending/expired rows only *inside* the chosen mutating route (never before classification — a `blocked-anomalous` or early-return outcome must never destroy the invite).
