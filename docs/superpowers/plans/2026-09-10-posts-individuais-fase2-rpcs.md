...
| `workflow_etapas_inconsistent` | detach (zero ou duas etapas `ativo`, ou `ordem` duplicada entre as etapas do fluxo de origem) |
| `template_not_found` | apply |
| `template_empty` | apply |
| `template_invalid` | apply (`workflow_templates.etapas` malformado: `nome` vazio, `tipo`/`tipo_prazo` fora do domínio, ou `prazo_dias`/`responsavel_id` que não é um inteiro não negativo dentro da faixa da coluna de destino) |
...
| `step_deadline_required` | detach (etapa futura com `data_limite` ausente de `p_step_deadlines`), apply (modo `data_fixa`/`data_entrega` com etapa de `ordem` maior ou igual à inicial sem `prazo_efetivo` no override) |
...
| `process_not_active` | transition (`avancar`/`voltar`/`concluir`), update_step (editar etapa de um processo que não está `ativo`) |
...
| `request_not_found` | detach (`request_id` de outra conta; também a colisão rara de `request_id` entre contas, quando o `INSERT` do recibo esbarra em `unique_violation` porque a checagem `EXISTS` do passo 1 não viu a linha ainda não commitada de outra conta) |
...
**Nota da rodada final do review:** `reorder_fluxos_board` (Task 8) também passou a tomar `:post_move`, mas por um motivo diferente do ciclo descrito acima: ela nunca insere em `post_process_events` nem trava linha de post, então não fecha o ciclo desta decisão. A razão é outra: `apply_post_process` e `detach_posts_keeping_process` calculam `MAX(board_position) + 1` sob essa mesma chave para o processo que criam ou reabrem, e um reorder concorrente sem o advisory podia gravar a mesma posição enquanto esse cálculo ainda estava em voo, empatando o espaço de índices do quadro. Com o advisory as três serializam. A frase "sete RPCs" acima ficou literalmente verdadeira só a partir desta rodada: até aqui eram seis, `reorder_fluxos_board` era a exceção deliberada (cabeçalho original da Task 8), e o motivo de deadlock desta decisão continua não se aplicando a ela.
...
- Códigos de erro: a tabela da seção Interfaces tem **47 códigos**, com `request_mismatch` (Decisão 28), `data_entrega_requires_approval_step` (Decisão 18) e `pending_steps_remaining` (Decisão 12) acrescentados nas três revisões do plano, e `template_invalid` e `step_deadline_required` acrescentados na execução (ver seção Execucao abaixo). Com os blocos de erro de argumento (87.6, 87.9, 87.10, 88.4b, 88.5, 88.6, 88.7, 89.6, 89.7, 89.8, 90.5, 90.7, 91.4 e 92.4) e o 87.8, **todos os 47 são levantados por pelo menos uma migration e checados por pelo menos um bloco de teste: zero códigos sem bloco.** `process_already_closed` agora tem bloco nos dois call sites, `remove` (90.5) e `attach` (91.4). O que continua parcial é a cobertura por *call site*, e a lista é exata, quatro linhas e nenhuma outra: `feature_disabled:feature_post_processes` tem bloco só em `detach` (87.4), não em `apply`; `post_not_found` tem bloco só em `detach` (87.6), não em `apply` nem em `attach`; `workflow_not_found` tem bloco em `detach` (87.6) e em `reorder` (92.1), não em `attach`; e `step_not_found` tem bloco só em `update_step` (90.1), porque o caminho homônimo de `transition` (nenhuma etapa `ativo`) é defensivo e o índice parcial `post_process_steps_one_active` o torna inalcançável com dado válido.
...
O SQL de cada task acima, no corpo do plano, fica como estava antes da implementacao: referencia pre-execucao, nao o texto final. O que foi de fato commitado diverge nos pontos abaixo, todos motivados por review interno, por review externo (Codex) ou por decisao do PO durante as rodadas de fix de cada task, nao por mudanca de escopo. Migrations `supabase/migrations/20260919000001..8_*.sql` e a suite completa de `supabase/tests/entitlements/85..92_*.sql` carregam o SQL exato.

- **Task 1** (`20260919000001_post_process_fingerprints.sql`). `etapa_atual` NULL serializa como string vazia (`etapa_atual=`), nao como `"0"`: o literal antigo colidia com um fluxo real de `etapa_atual = 0`. `workflow_fingerprint` desempata `string_agg` por `ORDER BY e.ordem, e.id`, porque `workflow_etapas` nao tem `UNIQUE (workflow_id, ordem)`. Fluxo inexistente continua distinto de `etapa_atual` NULL pela variavel `FOUND` do PL/pgSQL, nao pelo conteudo lido: a primeira derruba a funcao antes do `RETURN` final, a segunda so afeta a serializacao.
- **Task 2** (`20260919000002_post_process_hardening.sql`). O bloco 86.3 da suite precisou anular `profiles.active_workspace_id` explicitamente apos o `insert` em `auth.users`: `handle_new_user_workspace` auto-provisiona um workspace dono em todo insert bare naquela tabela, entao o caso "sem workspace ativo" exige o `update` para nao herdar um workspace novo por acidente. O mock Deno de `express-post-cleanup-cron_test.ts` foi atualizado para poupar tambem `estado = 'concluido'` (nao so `'ativo'`), acompanhando a mudanca de `express_cleanup_delete_avulso_drafts` (Decisao 25 da fase 1).
- **Task 3** (`20260919000003_detach_posts_keeping_process.sql`). `workflow_etapas` da origem passa a ser travada (`FOR UPDATE`) antes do calculo do fingerprint, prevenindo uma edicao concorrente de etapa entre o lock do fluxo e a leitura; o lock da linha do fluxo em si trocou de `FOR UPDATE` para `FOR NO KEY UPDATE` porque o `UPDATE` direto de etapa pelo CRM dispara um trigger que insere em `workflow_events`, cuja FK pega `FOR KEY SHARE` na linha do fluxo, e o par `FOR UPDATE`/`FOR KEY SHARE` fechava um ciclo de deadlock (40P01) que `FOR NO KEY UPDATE` evita sem abrir mao da exclusao mutua com outras RPCs de movimentacao. `ordem` duplicada entre as etapas do fluxo de origem agora levanta `workflow_etapas_inconsistent` em vez do `23505` cru do `UNIQUE` de `post_process_steps`. A chave de `p_step_deadlines` passou por uma regex mais estrita (`^(0|[1-9][0-9]{0,8})$`, sem zero a esquerda, no maximo 9 digitos) com o cast para `integer` protegido por `EXCEPTION`, cinto e suspensorio contra a regex mudar no futuro. Um `NULL` dentro do lote de `p_post_ids` levanta `post_ids_required` (`array_position` contra `NULL`); `v_wf.status <> 'ativo'` virou `v_wf.status IS DISTINCT FROM 'ativo'` para nao deixar um `status` nulo passar como ativo. `p_archive_empty_flow` entra no digest de idempotencia junto com `p_fingerprint` e o lote ordenado, por ser escolha do usuario no dialogo (fold decidido no review do plano, antes da implementacao). Etapa futura da origem com `data_limite` preenchido e omitida de `p_step_deadlines` agora levanta `step_deadline_required`, em vez de perder a data em silencio. Uma colisao rara de `request_id` entre contas diferentes (a checagem `EXISTS` do passo 1 nao ve a linha ainda nao commitada de outra conta) converge para `request_not_found` via `EXCEPTION WHEN unique_violation`, em vez do erro cru de `unique_violation` no INSERT do recibo.
- **Task 4** (`20260919000004_apply_post_process.sql`). Codigo novo `template_invalid`: um template com `nome` vazio, `tipo`/`tipo_prazo` fora do dominio, ou `prazo_dias`/`responsavel_id` que nao seja um inteiro nao negativo dentro da faixa de `integer`/`bigint` (respectivamente ate 9 e ate 18 digitos, sem sinal) levanta esse codigo antes de qualquer INSERT, em vez do `22P02`/`23514`/`22003` cru que o cast e o CHECK produziam. A chave de `p_step_overrides` ganhou a mesma regex mais estrita da Task 3 com cast protegido por `EXCEPTION`. O `INSERT INTO post_processes` passou a usar `RETURNING id, revisao, estado` para montar o retorno, em vez dos literais `'ativo'`/`1`. Nos modos `data_fixa` e `data_entrega`, toda etapa de `ordem` maior ou igual a inicial agora exige `prazo_efetivo` em `p_step_overrides` (`step_deadline_required` se faltar), nao so a etapa inicial. A regra estrutural do modo `data_entrega` (exigir uma etapa `aprovacao_cliente` na sequencia a partir da inicial) le so o `tipo` do jsonb ja travado por `FOR SHARE`, sem tocar em data nem reimplementar calculo de dias uteis, por decisao do PO (Decisao 18).
- **Task 5** (`20260919000005_transition_post_process.sql`). A RPC em si nao mudou nesta fase de fix, so a suite: a regra `pending_steps_remaining` (recusar `concluir` quando alguma etapa de `ordem` maior ainda esta `pendente`, para tornar o re-arm do proximo ciclo impossivel por construcao) e o bloco 89.8 vieram de uma decisao do PO tomada no review do plano antes da implementacao (spec 5.5). O fix round de suite corrigiu o bloco 89.3 (a etapa usada para provar "agendado nunca reinicia" tinha `tipo = 'padrao'` e nunca exercitava o ramo de re-arm; trocada para `aprovacao_cliente`), acrescentou o bloco 89.9 (flag `feature_post_processes` desligada nao bloqueia `avancar`/`voltar`/`concluir` sobre uma execucao ja existente, so a criacao) e expandiu o assert de retorno de 89.0 para o contrato completo (`ok`, `command`, `process_id`, `post_id`, `estado`, `steps` com o array inteiro).
- **Task 6** (`20260919000006_update_step_and_remove_process.sql`). `update_post_process_step` ganhou uma guarda de estado do processo: `process_not_active` quando `estado <> 'ativo'`, checada logo apos o `NOT FOUND` e antes da checagem de `revisao`. Sem ela, editar a etapa de uma execucao ja `encerrada` (por exemplo apos um `remove_post_process`) gravava responsavel/prazo, incrementava `revisao` e logava `etapa_editada` num processo terminal. Suite 90 ganhou os blocos 90.6 (`remove_post_process` sobre um processo `concluido`, caminho em que o `UPDATE ... WHERE estado = 'ativo'` do remove atinge zero linhas), 90.7 (editar etapa de um processo recem-`encerrado`, provando que a guarda nova dispara antes da checagem de `revisao`, chamando com a revisao desatualizada) e 90.8 (`remove_post_process` de outra conta responde `process_not_found`).
- **Task 7** (`20260919000007_attach_post_closing_process.sql`). No fix round 1 a migration nao mudou, so a suite 91. `91.0` passou a inserir um post baseline no fluxo antes do vinculo, para que o assert de `ordem = (maior ordem do fluxo) + 1` seja discriminante, e a checar o retorno completo (`ok`, `process_id`, `post_id`, `workflow_id`, `revisao`). Bloco novo 91.5 prova as duas bordas do limite de plano (`atual + 1 = limite` vincula, `atual = limite` recusa). Bloco novo 91.6 cobre fluxo e post de uma conta totalmente diferente (`workflow_not_found`/`post_not_found` pelo filtro `conta_id`, distinto do `post_belongs_to_another_client` de 91.2). Bloco novo 91.7 prova que `move_posts_to_existing_flow` e `move_posts_to_new_flow` caem no mesmo guard `post_a1_process_guard` que barra `attach_posts_to_flow`. Na rodada final do review a migration mudou: `v_wf.status <> 'ativo'` virou `v_wf.status IS DISTINCT FROM 'ativo'`, porque `workflows.status` e nullable e o operador antigo deixava um fluxo de status nulo passar como ativo (Minor 1, commit `3cdc5a29`); bloco novo 91.8 prova o caso.
- **Task 8** (`20260919000008_reorder_fluxos_board.sql`). O lado dos fluxos do reorder passou a exigir `status IS DISTINCT FROM 'arquivado'` na contagem de posse e no UPDATE, no mesmo espirito do filtro `estado IN ('ativo', 'concluido')` que ja existia do lado dos processos; sem isso um fluxo arquivado no payload teria sua `position` reescrita em silencio. A suite 92 ganhou os blocos 92.5 (fluxo arquivado da propria conta cai no mesmo codigo de posse que um id alheio) e 92.6 (processo `encerrado` da propria conta, mesmo codigo) e o bloco 92.3 passou a afirmar tambem que `service_role` tem `EXECUTE`. O fix round 1 ficou no commit `b81a6236` (nao no mesmo commit `5bdd9b33` da implementacao original da task). Na rodada final do review a funcao ganhou tambem `pg_advisory_xact_lock(hashtext(v_conta::text || ':post_move'))`, logo apos as checagens de `invalid_arguments` e antes dos dois blocos `FOR UPDATE`: `apply_post_process` e `detach_posts_keeping_process` calculam `MAX(board_position) + 1` sob essa mesma chave para o processo que criam ou reabrem, e um reorder concorrente sem o advisory podia gravar a mesma posicao enquanto esse calculo ainda estava em voo, empatando o espaco de indices do quadro; com o advisory as tres serializam (commit `207fb458`).
