...
9. Monitor: Stream dashboard spend, plus `post-media-cleanup-cron`'s own JSON response counters (`deleted`/`failed`/`streamIngested`/`streamSettled`/`streamReaped`/`streamErrors`). `post-media-cleanup-cron` is NOT wired into `reportCronFailure()` cron-failure triage today (unlike most other crons) — wiring it in is a separate, spawned follow-up task. Kill switch: unset the four non-cleanup secrets (playback/ingest stop; queued deletions keep draining). Full teardown: `purge-stream.ts`, then unset everything.
