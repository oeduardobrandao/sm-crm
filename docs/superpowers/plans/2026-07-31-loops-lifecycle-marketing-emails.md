...
**Why this task is load-bearing, not a fallback:** `FeatureGate` exists at only 5 call sites covering `feature_csv_import` and `feature_mcp`. Everything in `20260611140003_feature_triggers.sql` — `feature_hub_portal`, `feature_ideas`, `feature_financial`, `feature_contracts`, `feature_leads`, `feature_brand_customization`, `feature_custom_properties` — is a database trigger reached by a direct client write, with no edge function in the path.

`handleEntitlementMutationError` and `mapEntitlementError` are the right primitives — `mapEntitlementError` already parses both the DB-trigger shape (`"feature_disabled:feature_hub_portal"` in the PostgREST message) and the edge-function JSON shape (`{ error, feature }`).

> **CORRECTED AFTER IMPLEMENTATION.** This task originally claimed that wiring `handleEntitlementMutationError` into the global `MutationCache.onError` in `App.tsx` would cover all seven trigger-gated features. **It covers none of them.** `useMutation` exists at only seven non-test CRM call sites — `hooks/useBanners.ts`, `hooks/useNotifications.ts`, `pages/configuracao/tabs/RelatoriosTab.tsx`, `pages/configuracao/mcp/IntegracoesClaudePage.tsx`, and `pages/arquivos/{ArquivosPage,components/FileGrid,components/FileUploader}.tsx` — and not one of them is a trigger-gated surface. Leads, financeiro, contratos, ideias and marca write through plain `store/*.ts` async functions called from event handlers, so their error never reaches the mutation cache. The global hook is still worth keeping (it catches edge-function `{ error, feature }` denials), but the actual coverage comes from two explicit sources:
>
> 1. **`UpgradeLockedScreen`** (`components/paywall/UpgradeLockedScreen.tsx`), which `ProtectedRoute` now passes the flag to. This is the COMMON path for `feature_leads`, `feature_financial`, `feature_contracts` and `feature_ideas`: a user whose flag is off is routed here before they can reach the page that performs the gated write. It also picks up `feature_analytics_reports` and `feature_post_scheduling`.
> 2. **Explicit `handleEntitlementMutationError(e, <workspace id>)` calls in the catch blocks** of `HubTab` (hub token, `BrandEditor.save`, `uploadLogo`), `LeadsPage.onSubmit`, `FinanceiroPage.onSubmit`, `ContratosPage.onSubmit` and `PropertyDefinitionPanel.save`. These are the only observation point for `feature_hub_portal`, `feature_brand_customization` and `feature_custom_properties` (none of which has a route gate), and the stale-entitlement backstop for the three route-gated features.
>
> `upsertHubBrand` in `store/hub.ts` had to start throwing before (2) was possible for `feature_brand_customization`: it discarded the PostgREST `error` entirely, so a save the database had refused still reported "Marca salva!".
>
> **`feature_ideas` is covered by the route gate ONLY, and this is not fixable in the CRM.** `trg_feature_ideias` fires on `insert on ideias`; the CRM only SELECTs and UPDATEs that table (`store/ideias.ts`). Ideas are created by the client in the Hub via the `hub-ideias` edge function, which has no authenticated CRM user to attribute a `paywall_hit` to. There is no CRM catch site to instrument.
