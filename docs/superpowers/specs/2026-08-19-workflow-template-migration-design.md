...
- **modo_prazo:** o fluxo adota o `modo_prazo` do template destino. O
  ...
  existente (`computeDeliveryDeadlines` para `data_entrega`, usando o
  `dia_entrega` do cliente; `data_fixa` nasce sem datas para preenchimento
  manual; `padrao` não usa datas). O cliente monta o array completo de etapas
  novas e envia ao backend; não se reimplementa cálculo de dias úteis em SQL.
- **Posts:** nenhuma escrita em `workflow_posts`. O registry de custom status é
  por workspace, não por template, então nada quebra.
- **Propriedades personalizadas:** valores em `post_property_values` cujas
  ...
  destino. Os demais valores são apagados (listados no diálogo antes, com
  confirmação). `workflow_select_options` (FK para definição do origem) seguem a
  mesma regra: remapeadas quando a definição casou, apagadas senão.
...
- **Validações (dentro da transação):**
  ...
  - shape de cada etapa: `nome` não-vazio, `prazo_dias` inteiro >= 0,
    ...
    `status` coerente com `p_active_ordem`, `responsavel_id` nulo ou membro da conta;
...
- **Passos (uma transação, ordem):**
  1. remap de `post_property_values`: para cada definição do template origem com
     par no destino (nome+tipo), atualizar `property_definition_id` para a
     definição do destino, limitado aos posts deste workflow. Em conflito com
     `UNIQUE(post_id, property_definition_id)` (post já tem valor na definição
     destino), o valor existente no destino vence e o de origem é descartado;
  2. mesmo remap para `workflow_select_options` deste workflow;
  3. delete dos valores/options órfãos (definições sem par), limitado aos posts
     e ao workflow em questão;
  4. delete + insert da escada (`workflow_etapas`);
  5. update do workflow: `template_id`, `etapa_atual`, `modo_prazo`;
  6. insert em `audit_log`: `action = 'workflow.template_migrated'`,
     `resource_type = 'workflow'`, `resource_id = p_workflow_id`, `metadata` com
     template origem/destino, snapshot da escada antiga e nomes das propriedades
     descartadas. O `SECURITY DEFINER` cobre o insert (RLS de `audit_log` é
     service_role-only).
- Qualquer falha = rollback total.
- Duas definições homônimas do mesmo tipo no destino: casa com a de menor
  `display_order` (determinístico).
...
- `apps/crm/src/store/workflows.ts`:
  ...
  - helper puro `matchPropertyDefinitions(defsOrigem, defsDestino)` (nome+tipo,
    menor `display_order` em empate) usado pela prévia do diálogo — espelha a
    regra do SQL;
...
- **psql** (`supabase/tests/entitlements/`, roda no job `entitlement-tests`):
  posse entre contas, rollback em validação falha, remap nome+tipo, descarte de
  órfãos, `workflow_select_options`, conflito UNIQUE, gravação do audit_log.
