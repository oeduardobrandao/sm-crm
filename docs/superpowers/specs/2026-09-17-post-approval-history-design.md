...
**Data:** 2026-09-17 (revisado cinco vezes após review externa)
...
- **Motivo de ajuste:** tag obrigatória só quando `action = 'correcao'` — constraint condicional no banco (`CHECK`), não só validação client-side, para não permitir linha sem categoria via chamada direta à RPC. A constraint tem que validar tanto presença quanto valor: `CHECK (action <> 'correcao' OR motivo IN ('legenda', 'imagem_video', 'data', 'outro'))` (o `IN` já rejeita `NULL`, string vazia e qualquer valor fora dos quatro chips — checar presença sozinho, como uma versão anterior deste documento propôs, deixaria `''` ou texto arbitrário passar). **Mecânica da migration:** a coluna nova (`motivo text`, nullable) tem que ser adicionada primeiro, com esse `CHECK` criado como `NOT VALID` — um `CHECK` normal valida todas as linhas existentes no momento da criação, e linhas antigas de `correcao` sem motivo (que existem e vão continuar existindo, por decisão de escopo) fariam a migration falhar. Com `NOT VALID`, o constraint passa a valer só para linhas novas.
