...
- [ ] **Step 1: CobrancaPage.**
  ...
  - Manage block (lines ~184-214): branch on `subscription?.provider === 'pagarme'` → replace the portal button with two: `Atualizar cartão` (opens dialog `{mode:'update-card', plan:null}`) and `Cancelar assinatura` (shadcn `AlertDialog`; description: trialing → `Sua assinatura será cancelada agora, sem cobrança.`; else with `current_period_end` → `Seu acesso continua até {dd/MM/yyyy}. Depois disso, o workspace volta ao plano gratuito.`; confirm → `cancelPagarmeSubscription()` → `toast.success('Assinatura cancelada.')` + `startPlanRefetchPoll()`; error → `toast.error(err.message)`). Stripe rows keep the portal button untouched.
...
- [ ] **Step 2: ComecarPage.** In `startAndRedirect` (and the auto-intent effect path that calls it): when `interval === 'year'` && plan passes the gate → `captureCheckoutStarted(planId, 'year', 'onboarding', 'pagarme')` + open the dialog (same state shape; plan resolved from the loaded plans list; `source="onboarding"`; `trialEligible={!subscription?.hasEverSubscribed}` — reuse whatever subscription/hasEverSubscribed data the page already has, or default `true` if the page doesn't load subscription today: check and if absent, fetch via the existing `getWorkspaceSubscription` query idiom). `onSuccess` → `navigate('/dashboard?trial=started')` (matches the master plan) or the page's existing post-checkout destination. Copy line ~256: when year && gate → `depois 12x de {formatBRL(monthly)} no cartão`; unchanged otherwise.
...
- [ ] **Step 1: `plan-mutations.ts`.** Add `"pagarme_12x_enabled", "pagarme_plan_id_annual"` to `allowedScalar` (update path) AND to `handleCreatePlan`'s explicit `if (rest.x !== undefined)` whitelist block (both fields; without the create block the field silently doesn't save on create).
...
- [ ] **Step 3: tests.** plan-form round-trips: form → payload includes both fields ('' id → null); plan → form (null id → ''); extend any existing plan-mutations Deno test with: update accepts the two new scalars, create persists them (mirror the existing scalar assertions).
...
3. Flip checklist stays the Fase 5 gate + support GO; the admin checkbox is the per-plan switch.
