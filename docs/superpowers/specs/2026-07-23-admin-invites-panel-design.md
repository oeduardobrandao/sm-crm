...
1. Shows every invite for the workspace with enough enrichment to diagnose "the email never
   arrived" without touching SQL or the Supabase dashboard.
...
| `_shared/invite-actions.ts` | `findAuthUserByEmail`, `getAuthStatesByEmails`, `cancelInvite`, `resendInvite` | extracted from `invite-user/index.ts` (+ new batching helper) |
...
- `cancelInvite(svc, {inviteId, contaId})`: verifies the invite belongs to `contaId`, deletes the
  row, then runs the existing unconfirmed-user cleanup (classification-guarded: never deletes an
  onboarded user or the anomalous confirmed-no-profile state).
- `resendInvite(svc, {contaId, email, role, invitedBy, redirectBase})`: first deletes any
  existing `pending`/`expired` invite rows for the email + conta (as `invite-user` does today,
  preventing duplicate rows), then classification-routed —
  `resend-link` → `generateLink({type:'recovery'})` + `sendInviteEmail` (Resend); `reinvite` →
  delete user, fall through to new-user; new-user → `sendPendingWorkspaceInvite` (insert-first,
  rollback-on-send-failure semantics preserved); `add-direct` → **no action**, returns a distinct
  `already-onboarded` result (the admin path must never silently add someone to a workspace).
  Returns which route ran so callers can phrase the outcome.
...
- `invite-user/index.ts` keeps its own request handling, seat checks, and response shapes — it
  calls these primitives. Its existing deno suite is the regression net.
...
1. **`get-workspace-invites`** `{workspace_id}` → invite rows for the workspace, newest first,
   capped at 50. Each row: `{ id, email, role, status, created_at, accepted_at, expires_at,
   invited_by, silent_add, link_expired, auth_state }` where
   ...
   - `auth_state` = `{ user_exists, email_confirmed, has_password, onboarding_complete,
     ...
     emails.
2. **`admin-cancel-invite`** `{workspace_id, invite_id}` → `cancelInvite`, then
   `insertAuditLog({ action: 'admin-cancel-invite', conta_id: workspace_id, actor_user_id:
   <admin auth id>, resource_type: 'invite', resource_id: invite_id, metadata: { email } })`.
3. **`admin-resend-invite`** `{workspace_id, invite_id}` → allowed only for `pending`/`expired`
   rows (400 otherwise; 404 for not-found/wrong-workspace). Calls `resendInvite` preserving the
   **original** `invited_by` (the workspace owner remains the inviter of record). Audit action
   `admin-resend-invite` with the route taken in metadata. The `already-onboarded` result maps
   to a 200 with a distinct flag, not an error.
...
- Columns: **Email · Role · Status · Sent · Auth state · Actions**.
  ...
  - **Auth state**: one plain-language chip, first match in escalating order:
    `is_member` → "member of this workspace"; `has_password && onboarded` → "onboarded";
    `email_confirmed && !has_password` → "confirmed, no password";
    `user_exists` → "email sent, never opened"; else → "no account".
...
- Feedback via the page's existing toast/error pattern. The `already-onboarded` resend response
  renders as an info toast ("this person already has an account and needs no email"), not an
  error.

## Error handling

- Server: 404 invite not found / wrong workspace; 400 resend on `accepted`; GoTrue/Resend
  failures mid-resend keep `invite-user`'s rollback semantics (no phantom pending row) and
...
- **Deno**: moved-module tests keep passing with updated paths; new tests for
  `_shared/invite-actions.ts` (resend routing per classification incl. `already-onboarded`,
  `cancelInvite`'s refuse-to-delete-onboarded guard, `getAuthStatesByEmails` single-scan
  batching) and for the three `platform-admin` actions (admin gate, wrong-workspace rejection,
  status-based action rules), following the `plan-mutations` DI test pattern.
- **Frontend (RTL)**: `WorkspaceInvitesCard` — chip derivation across auth states, action
  visibility per status, confirm-dialog copy, refetch after action.
- **Gates before pushing**: `npm run test`, `deno test supabase/functions/`, `npm run lint`,
  `npm run format:check`; `git checkout -- deno.lock` after the deno run.
- **Live verification**: on prod, open Araripe MKT's workspace detail — the panel must show the
  historical `silent_add` rows and the typo invite as "no account / email sent, never opened"
  (wording per chip order above: the typo user exists unconfirmed → "email sent, never opened").
