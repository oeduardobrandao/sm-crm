...
**Prerequisite:** the correction-flow rework (`2026-09-17-hub-correction-flow-rework-design.md`, branch `claude/post-approval-history-5938fa`, PR #541, still open) **committed on that branch**. As of 2026-09-18 its card/hook/`hub-approve` changes are uncommitted working-tree edits; commit them first. The dialog's correction panel depends on `useEditSuggestion.dirty`, the optional-motivo `CorrectionReasonChips` and the new `shared.*` i18n keys. Build this as a branch stacked on `claude/post-approval-history-5938fa`; merging #541 first is not required.
...
- Overlays only: status pill top-left (a new `components/posts/StatusTag` reading `STATUS_COLORS` and `getPostPublishState`, both **moved from `PostagensPage.tsx` into `lib/postView.ts`** and exported), type glyph top-right (carousel / reel / story, lucide), title in one line on a bottom gradient. No date, no actions.
- Variants:
...
- Radix `Dialog` (`@radix-ui/react-dialog`, already a root dependency; the Hub has no Dialog primitive yet, so this adds `components/ui/HubDialog.tsx` wrapping Radix with the Hub's classes). **Portals into `.hub-root`** (Radix `Portal` `container` prop, falling back to `document.body` only if the element is missing), not `document.body`: `index.html` scopes every `.hub-*` rule as `.hub-root .hub-*`, and dark mode is `data-theme="dark"` set on `.hub-root` by `hooks/useTheme.ts`, so a body portal would render the dialog with no `hub-*` styling and no dark mode (the same reason `IdeiasPage`'s modal portals there). `.hub-root` itself carries no transform; the `.hub-fade-up` transform is on the page container below it, so `position: fixed` inside the portal is safe. `z-[9000]`; `PostMediaLightbox` keeps `z-[9005]` and opens above it.
...
- The per-card header row (`StatusTag`, `OpenPostLink`, `SharePostButton`) is removed: status is on the tile, share is in the dialog, `OpenPostLink` is redundant once the URL carries the post id. `OpenPostLink.tsx` and its test are deleted (the two pages were its only consumers); `lib/hubLinks.ts` stays for `SharePostButton`.
...
- `PostagemFocoPage.tsx` and its route entry are deleted. `router.tsx` uses **one route object per page with an optional segment**: `path: 'postagens/:postId?'` and `path: 'aprovacoes/:postId?'` (React Router v7 optional params). One `RouteObject` means the page component is not remounted when the URL gains or loses the id, so `selectedIds`, the filters, select mode and scroll position survive opening and closing the dialog. Two sibling entries (`postagens` + `postagens/:postId`) would remount the page on every tile click and are not acceptable.
...
- `PostagemFocoPage.tsx` and `pages/__tests__/postagemFocoPage.test.tsx`; `pages/__tests__/aprovacoesPostagensFeatures.test.tsx` (it mocks the deleted cards) is replaced by `postagensPage.test.tsx` and `aprovacoesPage.test.tsx`.
- `OpenPostLink.tsx` and `__tests__/openPostLink.test.tsx`.
