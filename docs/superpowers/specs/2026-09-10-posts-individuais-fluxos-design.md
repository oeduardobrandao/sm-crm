...
Ajustes da revisão 2.3 (2026-09-10), após as decisões do PO sobre o plano da fase 2: a limpeza de Express passa a poupar o rascunho avulso com **execução vigente**, `ativo` **ou** `concluido`, e não só `ativo`, porque a fase 2 cria o primeiro caminho para um processo chegar a `concluido` sem tirar o post de `rascunho` (§10 e critério 12.18, corrigidos nesta revisão); e a §7 fica como está, porque o PO manteve no servidor a exigência estrutural que ela descreve para o modo `data_entrega`, com `apply_post_process` recusando por `data_entrega_requires_approval_step` um template sem etapa `aprovacao_cliente` na sequência a partir da inicial. As decisões de `concluir` herdar o diálogo de aprovação (§5.5 e §6.2) e de `voltar` reabrir a etapa imediatamente anterior por `ordem` (§5.4) foram aprovadas como o documento já as descreve, sem mudança de texto.
...
| Limpeza de Express | A única GC de avulsos é o passo 3 de `express-post-cleanup-cron`: candidatos `is_express AND workflow_id IS NULL AND status = 'rascunho' AND created_at < cutoff`. O handler pré-filtra por `post_processes` (otimização, e já dá a maior parte da contagem `avulso_skipped_with_process`) e apaga pela RPC `express_cleanup_delete_avulso_drafts(bigint[])` (SECURITY DEFINER, só `service_role`, migration `20260918000004`): `FOR UPDATE` ordenado nas linhas candidatas, depois um `DELETE ... NOT EXISTS (execucao vigente: estado ativo ou concluido)` numa instrução nova, reconferindo os predicados de rascunho avulso express, e devolvendo os ids apagados. RLS não protege; é a RPC, não o pré-filtro, que garante que nenhum rascunho com execução vigente (`ativo` ou `concluido`) criada entre a leitura e o delete seja apagado. Execução `encerrado` não poupa o rascunho |
...
18. A limpeza de Express não apaga rascunho com execução vigente (`ativo` ou `concluido`). Flag desligada mantém execuções visíveis e operáveis.
