...
- Plain-text mention token syntax, everywhere: `@[Label](tipo:id)` for membro/cliente/tarefa and `@[Label](post:id:workflowId)` for posts (third segment = workflow id; may be absent in which case the chip renders without a link). Regex: `/@\[((?:(?!@\[).)+?)\]\((membro|post|cliente|tarefa):(\d+)(?::(\d+))?\)/g`. UPDATE (post-hoc, 2026-09): the label group was originally `([^\]]+)`, which cannot represent a label containing `]` -- real post titles do (e.g. `Carrossel - [AUTOMAÇÃO] ...`), so those tokens rendered as unclickable plain text. The label is now a lazy scan ended by the first `](tipo:id)` tail, with a `(?!@\[)` guard so a literal tail-less `@[...` fragment is not swallowed into a later token's label.
