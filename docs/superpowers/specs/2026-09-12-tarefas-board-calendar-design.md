...
- New file `apps/crm/src/pages/tarefas/views/BoardView.tsx` — the date-bucket board. **Correction from an earlier draft:** this does not reimplement its own `DndContext`/sensors/`DragOverlay` — it follows the exact same shape as `StatusKanbanView.tsx`/`MembrosBoardView.tsx`, which both just build a `BoardColumn[]` and an `onDropCard` handler (using `useOptimisticTarefas`) and render the existing, already-shared `<TarefaBoard columns={...} membros={membros} now={now} onCardClick={...} onDropCard={handleDrop} />` from `boardShared.tsx`. `BoardView` does the same, so it gets dnd-kit wiring, the drag overlay, and sensors for free instead of duplicating them.
...
4. Footer row (new): a small colored dot plus the client name (`cliente_nome`), replacing today's plain grey text under the title. **Correction from an earlier draft of this spec:** the dot must use the client's own `cor` column (`clientes.cor`), not a hash — `avatarColorClass`'s own doc comment explicitly says it's "for initials avatars (people, not clients — clients carry their own `cor` column)", and this exact `cliente.cor || 'var(--surface-hover)'` fallback pattern is already established across Entregas cards (`WorkflowCard.tsx:165`, `ListView.tsx:130`, `ConcludedView.tsx:153`, etc.) — reuse that convention (`tarefa.cliente_cor || 'var(--text-muted)'` for the dot). This requires a small store change: `getTarefas()`'s `clientes(nome)` embed becomes `clientes(nome, cor)`, and `TarefaWithRelations`/`TarefaRow` (`store/tarefas.ts`) gain `cliente_cor: string | null`. `cor` is already an exposed column on `clientes` (selected elsewhere, e.g. `store/clients.ts`), so this is not a new column through the `clientes` GRANT allowlist — just projecting an already-visible column through one more embed. Dot + client name are both omitted when `cliente_nome`/`cliente_id` is null, same as today.
