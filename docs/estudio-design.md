...
- **Optimistic concurrency**: `rev` is **trigger-maintained and excluded from the writable column grants** — writers never set it. They send `expected_rev` and the UPDATE is guarded `.eq('rev', expected_rev)`; the trigger bumps `rev` after the guard matches. Mismatch (0 rows) → structured conflict error naming the current rev. This lets `post-design-manage` run as the authenticated user under RLS (no service-role needed for saves).
- **RLS**: standard member check (`conta_id IN (SELECT public.get_my_conta_id())`) + service-role bypass — the exact `files`/`folders` pattern.
- **Column-level grants** (mcp_api_keys precedent): authenticated may INSERT/UPDATE only `(doc, doc_version, updated_via, updated_by)` — render-state columns and `doc_hash` move only via triggers and service role.
- **Render claim/finish RPCs** (SECURITY DEFINER, service_role only):
  ...
  - `finish_design_render(design_id, claimed_hash, success, error)` — **hash-guarded**: if `doc_hash` moved mid-render, lands on `pending` instead of `rendered` (never marks stale artifacts fresh); failure writes sanitized `render_error` only.
- `ALTER TABLE post_file_links ADD COLUMN IF NOT EXISTS origin text NOT NULL DEFAULT 'manual' CHECK (origin IN ('manual','design'));` — the discriminator that lets `design-render` replace only its own links while manual media (e.g. the reel video) survives on the same post.
- `ALTER TABLE hub_brand ADD COLUMN IF NOT EXISTS logo_file_id bigint REFERENCES files(id) ON DELETE SET NULL;` — the brand logo as a real file (the schema is `file_id`-only). Backfill lazily: first Estúdio/MCP use imports `logo_url` bytes into R2 and sets the column. The import is a server-side fetch of tenant-authored free text, so it is SSRF-hardened: https only, deny private/link-local hosts after DNS resolution, ≤5 MB, `image/*` content type + magic-byte sniff, 10 s timeout; the resulting file goes through `file_insert_with_quota` (charges tenant storage quota normally).
...
- **Quota counting rule: only `status='succeeded'` rows count** against the monthly limit — users never lose quota to our failures; `cost_usd_estimate` is logged on every row for internal cost accounting.
- Enforcement is two-layer: call-time pre-check (friendly 403 before paying Gemini) + `enforce_plan_count_limit` BEFORE-INSERT trigger backstop with predicate `status = 'succeeded' AND created_at >= date_trunc('month', now())` (advisory-lock race safety; in-flight pendings bounded by the burst limiter). Month boundary is UTC — documented, not fought.
...
- **Shared render core**: all satori/resvg/mozjpeg wasm init, font + emoji caches, and doc→satori-tree building live in `supabase/functions/_shared/design-render-core.ts` — one render stack with three consumers: `design-render` (full pages), the `mcp` function (measure pass §2.5 + `preview_design` §9), and tests.
...
- **Flow**:
  ...
  2. Chunked calls (`page_index > 0`) re-read the design row and abort (204) if `rev` changed or `render_status ≠ 'rendering'` — a mid-chain doc write kills the chain cleanly (the trigger already reset status to `pending`, and the sweep cron restarts it).
  ...
  4. Write JPEG to R2 `contas/{conta_id}/designs/{design_id}/{rev}/{page_id}.jpg`; insert `files` row via `file_insert_with_quota` (storage quota charged consistently).
  5. More pages → self-invoke `POST design-render { design_id, rev, page_index+1 }` and return; else finalize:
  6. Re-confirm `doc_hash` still equals the claimed hash, **then** rewrite `post_file_links` atomically: delete previous links `WHERE post_id = ? AND origin = 'design'`, insert new with `origin='design'` (`sort_order` = page index; `is_cover` for `reel_cover`). Manual links (`origin='manual'`, e.g. the reel video) are never touched. Old files hit the existing reference-count → `file_deletions` orphan queue.
  7. `finish_design_render(design_id, claimed_hash, true)` — hash-guarded (§4.1).
  Failures at any step → `finish_design_render(..., false, sanitized_error)`; raw satori/provider errors go to internal logs only.
- **Editor asset uploads are not post media**: images inserted as design layers upload via the existing presigned pipeline but finalize **without `post_id`** — layer assets are referenced by `file_id` inside the doc, never linked as post media. Only rendered outputs get `post_file_links` rows (always `origin='design'`).
...
User-facing edge function (JWT verification ON, `buildCorsHeaders`, profile→`conta_id` per the file-upload-finalize pattern). It is the only design write path for the CRM; runs as the authenticated user under RLS (`rev` is trigger-maintained, §4.1, so no service role is needed for saves).
...
| `PUT` `{post_id, doc, expected_rev}` | Feature gate `feature_estudio` → post status ∈ EDITABLE_STATUSES → shared validation §2.4 → guarded update `.eq('rev', expected_rev)` → fire-and-forget `design-render` trigger → `{design (normalized), rev, warnings, layout}` |
...
- **Autosave**: 800 ms-debounced TanStack mutation → `post-design-manage` (new edge function: user JWT, validates via the shared schema, guarded `rev` update, fires render trigger). Save states: `saved | saving | dirty | conflict`.
...
- `useSatoriRenderer` dynamic-imports satori + yoga wasm inside the route chunk (zero main-bundle cost; Vite handles `.wasm` via `new URL(..., import.meta.url)`).
...
- CI gates: `npm run format && npm run lint && npm run test && deno test supabase/functions/` before every push.

## 12. Rollout

1. Migrations (SQL editor procedure §4.4, both envs).
2. Fonts build + R2 upload (`fonts:build --upload`); commit generated manifest + public assets.
3. Edge deploys (`--use-api`; `--no-verify-jwt` only for `design-render`): `design-render`, `post-design-manage`, `generate-image`, `mcp`, plus `workspace-limits`/`platform-admin` (entitlements embed). Then `git checkout deno.lock && npm ci`.
4. Vercel (CRM + admin mirrors) last. Feature stays dark behind `feature_estudio` until enabled per plan.
5. Staging soak first: load-test design-render on real edge hardware (spike numbers are laptop-local — **R1**).
