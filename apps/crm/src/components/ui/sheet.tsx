...
const sheetVariants = cva('fixed z-[9011] gap-4 bg-background p-6 shadow-lg', {
  variants: {
    side: {
      ...
      bottom: 'sheet-pane--bottom inset-x-0 bottom-0 border-t',
      left: 'sheet-pane--left inset-y-0 left-0 h-full w-3/4 border-r sm:max-w-sm',
      right: 'sheet-pane--right inset-y-0 right-0 h-full w-3/4 border-l sm:max-w-sm',
    },
  },
  defaultVariants: {
    side: 'right',
...
interface SheetContentProps
  ...
  /** Extra classes for the overlay, useful when a sheet has breakpoint-specific visibility. */
...
const SheetContent = React.forwardRef<
  ...
  SheetContentProps
>(({ side = 'right', className, children, overlayClassName, style, ...props }, ref) => {
  const needsTopSafeArea = side !== 'bottom';
    ...
    <SheetPortal>
      ...
      >
        ...
        <SheetPrimitive.Close
          ...
          style={
            needsTopSafeArea ? { top: 'calc(1rem + env(safe-area-inset-top, 0px))' } : undefined
          }
        >
          <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn('text-xl font-bold tracking-[-0.05em] text-foreground', className)}
    {...props}
  />
));
