...
import { identifySupportUser, resetSupportChat } from '../lib/supportChat';
...
interface Profile {
  ...
  // Present because the profile query is `select('*')`. Declared explicitly so it narrows past
  // the index signature below -- the support chat passes it as the contact's phone number, which
  // is what lets the inbox merge a user's web-chat and WhatsApp conversations into one contact.
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (userChanged) {
        ...
        resetAnalytics();
        // Same shared-machine reasoning as resetAnalytics() above, but for the
        // support chat: without this, B's chat messages land on A's identified
        // contact (A's email/name), since the widget persists the identity in
        // the browser until told otherwise. The call is internally guarded --
        // a support-tooling failure must never break an auth transition.
        resetSupportChat();
        queryClient.clear();
  ...
  // Support-chat identification, split out from the profile-hydration effect
  // above on purpose: that effect is keyed on [sessionReady, userId] so an
  // email or name change alone never re-triggers the profile/membership fetch,
  // but the widget still needs to learn about exactly those changes. Without
  // its own effect, a Supabase USER_UPDATED event (e.g. after an email change)
  // would update `user.email` in state while `userId` stays the same -- the
  // hydration effect wouldn't re-run, and the widget would stay identified
  // with the stale email for the rest of the session.
  //
  // The widget boots anonymously, so the inbox otherwise shows no identity.
  // `phone` is passed when the profile already carries it (no query is added
  // for it) because it lets the inbox merge this user's web-chat and WhatsApp
  // conversations into one contact. identifySupportUser() is internally
  // guarded and buffers until the widget is ready, so an early call is safe.
  //
  // Ordering vs. reset: reset runs synchronously -- inside the
  ...
  useEffect(() => {
    if (!userId) return;
    if (!user?.email && !profile?.nome) return;
    identifySupportUser({
      id: userId,
      email: user?.email,
      name: profile?.nome,
  ...
  }, [userId, user?.email, profile?.nome, profile?.whatsapp]);
  ...
  const signOut = async () => {
    ...
    resetAnalytics();
    // Same shared-machine reasoning as resetAnalytics() above, but for the
    // support chat: without this, the next person on this browser inherits the
    // outgoing user's identified contact (their email/name) and their support
    // messages land on it. The call is internally guarded because a
