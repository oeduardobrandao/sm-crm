...
// Instagram normalizes width; these image sizing values are not blocking errors.
export const IMAGE_MIN_DIM = 320;
export const IMAGE_AR_MIN = 3 / 4;
// Recommended Story framing, not a minimum accepted aspect ratio.
export const STORY_IMAGE_AR_MIN = 9 / 16;
export const IMAGE_AR_MAX = 1.91;
export const STORY_VIDEO_MAX_BYTES = 100 * 1024 * 1024;
export const STORY_VIDEO_AR_MIN = 0.1;
export const VIDEO_MAX_WIDTH = 1920;
export const VIDEO_AR_MIN = 0.01;
export const VIDEO_AR_MAX = 10;
export const VIDEO_MIN_DURATION = 3;
export const VIDEO_MAX_DURATION = 900;
  ...
  for (const f of files) {
    if (f.kind === 'image') {
      ...
      if (f.width && f.height) {
        ...
        if (!opts?.forStories && (ar < IMAGE_AR_MIN || ar > IMAGE_AR_MAX)) {
          errors.push({
            file_id: f.id,
            message: `Proporção da imagem fora do permitido (3:4 a 1.91:1)`,
          });
        }
      }
      ...
      if (f.size_bytes > videoMaxBytes) {
        errors.push({
          file_id: f.id,
          message: `Vídeo excede ${opts?.forStories ? 100 : 300} MB (limite do Instagram)`,
        });
      }
