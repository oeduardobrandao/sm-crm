...
import {
  upsertPostPropertyValue,
  createWorkflowSelectOption,
  getWorkflowSelectOptions,
...
export function PropertyValue({
  ...
  const handleAddOption = async () => {
    ...
    if (!label) return;
    try {
      const created = await createWorkflowSelectOption(
        workflowId,
        definition.id!,
        label,
        '#94a3b8',
  ...
  const renderDisplay = () => {
    ...
    if (definition.type === 'checkbox') {
      return (
        <span style={{ fontSize: '0.82rem' }}>{(localValue as boolean) ? '☑ Sim' : '☐ Não'}</span>
      );
    }
    if (definition.type === 'select' || definition.type === 'status') {
    ...
    return (
      <span style={{ fontSize: '0.82rem' }}>{formatDisplayValue(definition, localValue)}</span>
    );
  };
