...
const CRON_NAME = "storage-autoclean-cron";

Deno.serve(createStorageAutocleanCronHandler({
  ...
  run: async (req: Request): Promise<Response> => {
    ...
    try {
      const deps: StorageAutocleanCronDeps = {
        db: svc as unknown as StorageAutocleanCronDeps["db"],
        nowMs: () => Date.now(),
        report: (detail) => reportCronFailure(svc, CRON_NAME, detail),
      };
      const result = await runStorageAutocleanCron(deps);
