...
export function createHubPostsHandler(deps: HubPostsHandlerDeps) {
  return async (req: Request): Promise<Response> => {
    ...
    const { data: posts } = await db
      .from("workflow_posts")
      .select("id, titulo, tipo, status, ordem, conteudo, conteudo_plain, scheduled_at, ig_caption, instagram_permalink, published_at, publish_error, platform, workflow_id, workflows(titulo, created_at)")
