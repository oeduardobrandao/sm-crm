// HMAC-signed OAuth state for the TikTok connect flow. A dedicated copy of
// instagram-integration/oauth-state.ts (not shared) — same signing scheme, but the
// payload and the oauth_states row both carry provider: 'tiktok' so a state minted here
// can never be replayed against the Instagram callback (or vice versa) even though both
// providers derive their HMAC key from the same TOKEN_ENCRYPTION_KEY secret.
...
export async function createSignedState(clientId: string, userId: string, contaId: string, serviceClient: any): Promise<string> {
  ...
  const payload = JSON.stringify({ clientId, userId, contaId, provider: 'tiktok', nonce: crypto.randomUUID(), iat: Date.now() });
...
export async function verifySignedState(state: string): Promise<{ clientId: string; userId: string; contaId: string; nonce: string }> {
  ...
  if (parsed.provider !== 'tiktok') throw new Error('Invalid state provider');
