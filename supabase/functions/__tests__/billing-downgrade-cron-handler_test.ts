...
interface DbFx {
  // Leg A: due-rows read (workspace_subscriptions, no `.not`) and the flag-flip update.
  ...
  // Leg B: stale-attempts read (pagarme_checkout_attempts, no `.not`) and the expiry update.
  staleRows?: Array<{ id: string; workspace_id: string; pagarme_subscription_id: string | null }>;
  staleReadError?: { message: string } | null;
  expire?: (
    filters: Array<[string, string, unknown]>,
  ) => { data: unknown; error: { message: string } | null };
  // Leg C: local link-set reads (both use `.not`).
 ...
 * appended to `events` in call order, with its eq/is/not filters and whether `.not` was used
 * (the only way to tell leg A/B's plain reads apart from leg C's link-set reads on the same
 * two tables), so tests can assert sequencing, CAS pins, and which query fixture to serve.
 */
function makeDb(fx: DbFx & { events: Ev[] }, seq: { n: number }) {
  const from = (table: string) => {
    ...
    const settle = () => {
      fx.events.push({ table, op, values, filters, sawNot, abortSignal: sawAbortSignal, seq: seq.n++ });
      if (table === "workspace_subscriptions" && op === "read") {
        if (sawNot) return { data: fx.linkedRows ?? [], error: fx.linkedReadError ?? null };
      ...
      if (table === "pagarme_checkout_attempts" && op === "read") {
        if (sawNot) return { data: fx.pendingRows ?? [], error: fx.pendingReadError ?? null };
...
Deno.test("8. gateway: null -> leg B expires only no-sub-id attempts, leg C skipped, remoteSkipped:true, leg A still runs", async () => {
  ...
  // Leg C never even reads its local link sets.
  const legCReads = events.filter((e) => e.sawNot);
