-- Adds `total_clients` to admin_list_workspaces: how many clients exist across the
-- whole *filtered* set of workspaces, for the admin dashboard's "Total Clients" KPI.
-- Same filtered-set counting as total_members (20260810000001) — summing the page's
-- per-row client_count would under-count past the page size. Follow-up migration
-- rather than an edit of 20260810000002 because that version may already be recorded
-- in a database's migration history, where a same-version edit is silently skipped.
