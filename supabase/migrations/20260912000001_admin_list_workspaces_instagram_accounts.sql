-- admin_list_workspaces v7: totals gain 'total_instagram_accounts' and
-- 'total_instagram_accounts_active'.
--
-- Body is v6 (20260910000001) verbatim, plus TWO keys in the totals object so the admin
-- Dashboard can show how many Instagram accounts are connected across the platform in place
-- of the client count. Same signature, same grants; no DROP needed. A frontend deployed
-- before this migration keeps working (it just never reads the new keys).
--
-- Counting mirrors workspace_usage() (20260808000001): instagram_accounts is reached through
-- clientes.conta_id, and the unfiltered count includes revoked/expired links because they
-- still occupy a plan slot until removed. The '_active' variant is the subset whose
-- authorization is still good, so the Dashboard can flag connections that need reauthorizing.
-- Counted over the filtered id set rather than as a per-row column so the join runs once for
-- the whole page instead of once per workspace.
...
SELECT jsonb_build_object(
  ...
  'total_instagram_accounts',        (SELECT count(*)
                                        FROM instagram_accounts ia
                                        JOIN clientes c ON c.id = ia.client_id
                                       WHERE c.conta_id IN (SELECT id FROM filtered)),
  'total_instagram_accounts_active', (SELECT count(*)
                                        FROM instagram_accounts ia
                                        JOIN clientes c ON c.id = ia.client_id
                                       WHERE c.conta_id IN (SELECT id FROM filtered)
