-- Adds `total_with_overrides` to admin_list_workspaces: how many workspaces in the
-- whole *filtered* set have plan overrides (same predicate as the per-row
-- has_overrides flag). The admin dashboard's "With Overrides" KPI previously
-- counted only the fetched page, under-counting past the page size — the same
-- bug 20260810000001 fixed for total_members. Follow-up migration rather than an
-- edit of 20260810000001 because that version may already be recorded in a
-- database's migration history, where a same-version edit is silently skipped.
...
-- CREATE OR REPLACE preserves the ACL, but restate it so a fresh database gets
-- the same posture even if the earlier migrations' grants change.
