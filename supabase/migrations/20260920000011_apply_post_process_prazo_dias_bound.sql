-- supabase/migrations/20260920000011_apply_post_process_prazo_dias_bound.sql
-- FIX ROUND 4 de apply_post_process (20260919000004). A validacao de forma de
-- prazo_dias por etapa do template (FIX ROUND 2, regex '^[0-9]{1,9}$') so
-- garante inteiro nao negativo que cabe em integer -- nao limita o valor. Um
-- prazo_dias absurdo (ate 999999999) passa direto e trava computeDeadlineDate
-- no CRM (loop de 1 iteracao por dia util quando tipo_prazo = 'uteis') para
-- qualquer um que abrir o board ou o drawer desse post.
--
-- Mesmo teto ja aplicado a update_post_process_step em
-- 20260920000003_update_post_process_step_prazo_dias.sql (que por sua vez
-- espelha PostProductionSection.tsx MAX_PRAZO_DIAS): 0-999. Aqui isso vira
-- so um aperto na regex, de 9 para 3 digitos -- 1 a 3 digitos sem zero a
-- esquerda cobre exatamente 0-999, e o teste de nao-negativo (a regex nunca
-- casa um sinal '-') continua o mesmo.
--
-- Assinatura da funcao nao muda -- CREATE OR REPLACE basta, sem DROP.
...
BEGIN
  ...
  FOR v_etapa IN SELECT e.val FROM jsonb_array_elements(v_tmpl.etapas) AS e(val) LOOP
    ...
    -- bigint.
    -- FIX ROUND 4: 9 digitos cabe em integer, mas ainda deixa passar um
    -- prazo_dias absurdo (ate 999999999) que trava computeDeadlineDate no
    -- CRM em 'uteis'. Teto de 3 digitos = 0-999, mesmo limite de
    -- update_post_process_step (20260920000003) e de
    -- PostProductionSection.tsx MAX_PRAZO_DIAS.
    IF jsonb_typeof(v_etapa -> 'prazo_dias') IS NOT NULL
      ...
      IF jsonb_typeof(v_etapa -> 'prazo_dias') <> 'number'
         OR (v_etapa -> 'prazo_dias') #>> '{}' !~ '^[0-9]{1,3}$' THEN
        RAISE EXCEPTION 'template_invalid' USING ERRCODE = 'P0001';
      END IF;
    END IF;
