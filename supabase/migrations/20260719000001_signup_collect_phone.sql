-- Persist the phone number collected at self-signup.
-- The register form now sends `telefone` in the auth metadata; carry it into
-- profiles.telefone so account information can be delivered via SMS / WhatsApp.
-- Mirrors the latest handle_new_user_workspace (20260713000001) and only adds
-- the telefone column to the new-signup (owner) branch. Invited users do not
-- provide a phone through the invite flow, so that branch is unchanged.
