...
-- ---------------------------------------------------------------------------
-- resolve_ica_workflow_post_target(): copy-forward verbatim da definicao
-- atual em 20260830000002_avulso_claim_reorder_ica.sql (linhas 377-466), com
-- UM bloco novo inserido entre o fim do bloco do tombstone (linha 407) e o
-- early return (linha 409). O trigger ica_a1_resolve_workflow_post_target
-- (BEFORE INSERT OR UPDATE OF workflow_post_id, ig_media_id, ativo,
-- client_id, pending_post_deleted_at) ja existe e nao e recriado aqui:
-- CREATE OR REPLACE FUNCTION preserva o oid e o trigger passa a apontar para
-- esta definicao nova sozinho.
-- ---------------------------------------------------------------------------
