-- Generalize the atomic post-schedule reorder so both the client Hub and the CRM
-- (entregas drawer grid) drive one implementation. The core function is identical
-- to the original hub_reorder_post_schedules except the status allowlist is now a
-- parameter — the ownership locking, agendado future-slot guard and prepared-
-- container clearing (which protect instagram-publish-cron) stay shared, not
-- duplicated. The Hub keeps its exact client-facing allowlist via a thin wrapper.
--
-- p_updates is a JSON array of { "post_id": <bigint>, "scheduled_at": <timestamptz|null> }.
...
-- The Hub wrapper now delegates to the generalized core with its unchanged,
-- client-facing allowlist. Behaviour identical to the original function.
...
AS $$
  SELECT reorder_post_schedules(
    ...
    p_conta_id,
    p_updates,
    ARRAY['enviado_cliente', 'correcao_cliente', 'aprovado_cliente', 'agendado']
  );
$$;
