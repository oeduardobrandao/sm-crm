...
async function runSync(env: Env): Promise<{ srcObjects: number; copied: number; pending: number; errors: number }> {
  ...
  let copied = 0;
  let errors = 0;
  for (const key of missing.slice(0, MAX_COPIES_PER_RUN)) {
    try {
  ...
  const pending = Math.max(0, missing.length - MAX_COPIES_PER_RUN);
